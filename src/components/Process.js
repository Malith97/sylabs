    ...
    return (
        <>
            <div className="container">
                ...
                <p className="text-center text-black font-medium lg:my-4 xs:my-4 font-sans">Develop software that gives you a competitive edge using the latest technologies</p>
                <div className="grid lg:grid-cols-3 sm:grid-cols-3 lg:gap-4 sm:gap-3 xs:grid-cols-3 md:grid-cols-3 md:gap-4 xs:gap-4">
                    {processcards.map((val, key) => {
                        return (
                            <div className="bg-white rounded-lg lg:px-6 py-6 lg:my-8 transition duration-500 ease-in-out transform hover:-translate-y-1 hover:scale-105 static" key={key}>
                                <div className="static">
                                    <h1 className="font-black lg:text-9xl md:text-9xl text-black text-bold absolute lg:top-16 lg:left-16 opacity-10 sm:text-7xl xs:text-6xl">{val.No}</h1>
                                </div>
                                ...
                                <h4 className="font-medium mb-3 lg:text-3xl xs:text-md font-sans md:text-2xl">{val.cardHeading}</h4>
                                <p className="text-left xs:text-xs sm:text-sm md:text-md font-sans lg:text-md">{val.cardDesc}</p>
                            </div>
                        )
                    })}
                </div>
